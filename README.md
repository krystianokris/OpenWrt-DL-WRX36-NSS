...
SpecValueRouterDynalink DL-WRX36PlatformQualcomm IPQ807x / IPQ8072ACPU4× Cortex-A53 @ up to 2.2 GHzRAM1 GBEthernet4 × 1 GbE + 1 × 2.5 GbEWirelessQualcomm Wi-Fi 6 (ath11k)USBUSB 3.0StorageNAND
...
ItemValueBaseAgustinLorenzo OpenWrt NSSTargetqualcommax/ipq807xDevicedynalink_dl-wrx36ProfileDL-WRX36 NSS Performance EditionBuildsGitHub Actions (manual workflow)
...
Do not add every optimisation blindly — extra daemons and aggressive tweaks can cost RAM/IRQs without gain.
Measure before changing — LAN↔WAN, 2.5 GbE, PPPoE, concurrent flows, Wi-Fi→WAN, WireGuard, latency under load, CPU + ECM.
Protect the NSS path for max throughput; test SQM separately (best bufferbloat ≠ highest Mbps).
No aggressive IRQ pinning by default — wifi-nss-tuning only sets nss_offload=1.
...
Done (recent)
 ...
 Per-core CPU load on dashboard
 Live WAN RX/TX rates
 Dynamic WAN / PPPoE interface detection
 JSON escaping for ECM exception dumps
 Remove mesh / BATMAN from image and UI
 Single LuCI dashboard entry (no duplicate tab)
 Safe wifi-nss-tuning (no IRQ affinity)
