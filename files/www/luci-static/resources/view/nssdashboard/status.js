		...
		function renderDashboard(data) {
			...
			container.appendChild(
				grid([
					...
					card('RX rate', formatRate(d.wan_rx_rate)),
					card('TX rate', formatRate(d.wan_tx_rate)),
					card('WAN RX', formatBytes(d.wan_rx)),
					card('WAN TX', formatBytes(d.wan_tx)),
					card('L3 if', d.wan_l3),
					card('Device', d.wan_if),
			...
			container.appendChild(
				grid([
					card('Pending acceleration', pendingAccel, numberColor(pendingAccel)),
					card('Pending deceleration', pendingDecel, numberColor(pendingDecel)),
					card('IPv4 accel', d.ecm_pending, numberColor(d.ecm_pending)),
					card('IPv4 decel', d.ecm_pending_decel, numberColor(d.ecm_pending_decel)),
					card('IPv6 accel', d.ecm_pending6, numberColor(d.ecm_pending6)),
					card('IPv6 decel', d.ecm_pending_decel6, numberColor(d.ecm_pending_decel6))
				])
			);
			...
			container.appendChild(
				grid([
					card('IPv4 active exceptions', ipv4Active, warningNumberColor(ipv4Active)),
					card('IPv6 active exceptions', ipv6Active, warningNumberColor(ipv6Active))
				], 6)
			);
